...
- If you want to contribute via a one-time donation and want to make sure
  ...
  set this up at a later date.

I'd like a certain reward (e.g. a t-shirt) but I'd prefer making a smaller
monthly donation. Can I upgrade via a one-time donation?::
    It's planned to set up a donation page to allow this at a later date. Note
    that you can switch tiers on GitHub sponsors at any time.
...
would include qutebrowser pens, notebooks (the paper kind) or USB-sticks (for
the "expensive swag" reward).
