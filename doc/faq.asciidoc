...
How do I open git repositories from e.g. Github and GitLab in NeoVim?::
    You can easily add a key binding to open git repositories inside Neovim:
+
----
:bind ,. spawn kitty -e xdg-open "nvim-gitdev://open/?repo={url}"
----
+
The comma prefix is used to make sure user-defined bindings don't conflict with
the built-in ones.
+
Note that you need an additional package 
https://github.com/moyiz/git-dev.nvim/[git-dev.nvim] and enable the XDG config to view git repositories directly in NeoVim.
+
