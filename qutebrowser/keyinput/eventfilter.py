...
from qutebrowser.qt.core import pyqtSlot, QObject, QEvent, qVersion, Qt, QTimer, QRectF
...
class IMEEventHandler(QObject):
    ...
    _last_seen_rect: Optional[QRectF] = None
    ...
    def cursor_rectangle_changed(self) -> None:
        """Handler for when rectangle changes.

        Event handler for cursorRectangleChanged
        """
        # todo:
...
def init() -> None:
    ...
    if config.val.input.better_input_detection:
        def donothing() -> None:
