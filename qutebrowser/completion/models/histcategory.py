...
class HistoryCategory(QSqlQueryModel):
    ...
    def __init__(self, *, delete_func=None,
        ...
        self.empty_prefix = None  # type: typing.Optional[str]
    ...
    def set_pattern(self, pattern):
        ...
        """
        raw_pattern = pattern
        if (self.empty_prefix is not None and raw_pattern.startswith(
                self.empty_prefix)):
            log.sql.debug('Skipping query on {} due to '
                          'prefix {} returning nothing.'
                          .format(raw_pattern, self.empty_prefix))
            return
        ...
        if self.rowCount() == 0 and not self.canFetchMore():
            self.empty_prefix = raw_pattern
