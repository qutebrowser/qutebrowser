...
CHANGED_KEY_COMMANDS = [
    ...
    (re.compile(r'^yank -t(\S+)'), r'yank -\1 -- {title}'),
    (re.compile(r'^yank -t'), r'yank -- {title}'),
    (re.compile(r'^yank -d(\S+)'), r'yank -\1 -- {domain}'),
    (re.compile(r'^yank -d'), r'yank -- {domain}'),
    (re.compile(r'^yank -p(\S+)'), r'yank -\1 -- {url:pretty}'),
    (re.compile(r'^yank -p'), r'yank -- {url:pretty}'),
    (re.compile(r'^yank-selected -p'), r'yank -s -- {selection}'),
    (re.compile(r'^yank-selected --sel'), r'yank --sel -- {selection}'),
    (re.compile(r'^yank-selected'), r'yank -- {selection}'),

    (re.compile(r'^yank (url|title|domain|selection)$'), r'yank -- {\1}'),
    (re.compile(r'^yank -([sk]*) (url|title|domain|selection)$'),
     r'yank -\1 -- {\2}'),
    (re.compile(r'^yank (url|title|domain|selection) -([sk]*)$'),
     r'yank -\2 -- {\1}'),
    (re.compile(r'^yank pretty-url$'), r'yank -- {url:pretty}'),
    (re.compile(r'^yank -([sk]*) pretty-url$'), r'yank -\1 -- {url:pretty}'),
    (re.compile(r'^yank pretty-url -([sk]*)$'), r'yank -\1 -- {url:pretty}'),
