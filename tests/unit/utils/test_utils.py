...
def test_version_check(monkeypatch, qversion, compiled, pyqt, version, exact,
    ...
    """
    monkeypatch.setattr(utils, 'qVersion', lambda: qversion)
    if compiled is not None:
        monkeypatch.setattr(utils, 'QT_VERSION_STR', compiled)
        monkeypatch.setattr(utils, 'PYQT_VERSION_STR', pyqt)
        compiled_arg = True
    else:
        compiled_arg = False

    actual = utils.version_check(version, exact, compiled=compiled_arg)
    assert actual == expected


def test_version_check_compiled_and_exact():
    with pytest.raises(ValueError):
        utils.version_check('1.2.3', exact=True, compiled=True)
