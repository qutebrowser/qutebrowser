...
class TestYaml:
    ...
    pytestmark = pytest.mark.usefixtures('config_tmpdir', 'config_stub')
    ...
    def test_yaml_config(self, yaml, autoconfig, old_config, insert):
        ...
        if not insert and old_config is None:
            ...
            if insert:
                assert lines[1].startswith('# DO NOT edit this file by hand,')
    ...
    def test_autoconfig_loaded(self, yaml, autoconfig, autoconfig_loaded):
        autoconfig.write({})
        yaml.load()
        config.instance.yaml_loaded = autoconfig_loaded
        yaml.set_obj('tabs.show', 'never')
        yaml._save()
        lines = autoconfig.read_raw().splitlines()
        if autoconfig_loaded:
            assert lines[0] == '# Autoconfig is read'
        else:
            assert lines[0] == '# Autoconfig is NOT read'
